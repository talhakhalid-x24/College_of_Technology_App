...
class Company extends React.Component{
    ...
    onPress(){
        ...
        Firebase.database().ref('register_company').orderByChild('email').equalTo(this.state.email).once('value' ,(snapshot) =>{
        ...
        // console.log(email)
                if(email == this.state.email){
                    if(password == this.state.password){
                        this.props.navigation.navigate('EmpProfile',{
                                name:name,
                                email:email,
                                location:location,
                                phone:phone,
            ...
            <View style={styles.container}>
                <View style={styles.form}>
                    ...
                    <View style={{flexDirection:'row',marginVertical:20}}>
                        ...
                        <TouchableOpacity
                            activeOpacity={0.8}
                            onPress={() => this.props.navigation.navigate('Company_Register')}
                        >
